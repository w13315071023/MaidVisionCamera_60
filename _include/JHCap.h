...
typedef int (CALLBACK * CAPTURE_FRAME_PROC)(unsigned char *pImageBuffer, int width, int height, int format, void* handler);
...
Description: Get the version of firm
...
Parameters: *last_error, the index of the last error
Return: returns API_OK on success, otherwise returns API_ERROR.
Note: 
 --------------------------------------------------------------*/
...
/*==============================================================
Name: CameraReset
Description: Reset the data transfer pipe. If the data transfer is interrupt
             by noise
Parameters: device_id, the camera index
Return: returns API_OK on success, otherwise returns API_ERROR.
Note:
...
Description: Save the picture of different fomat,such as 24bit BMP,8bit BMP and JPEG
Parameters: device_id, the camera index
...
Description: Create a named fomat file from the buffer
Parameters: fileName, the path and name that the image will save as
            dataBuf, RGB Buffer
...
extern "C" DLL_EXPORT API_STATUS __stdcall CameraPlay(int device_id,HWND hWnd,CAPTURE_FRAME_PROC proc,void* handler);
